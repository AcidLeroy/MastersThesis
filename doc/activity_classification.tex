...
are ultimately the features that use to train and classify using an SVM. However,
...
threshold value used was empirically calculated from doing multiple runs. For
the dataset that we used, we found we got the best results by only retrieving
optical flow vectors with a magnitude greater than 75\% of the max value. The set
of Equations in \ref{eq:optical_threshod} illustrate this idea.

\begin{align}
  \label{eq:optical_threshold}
  \begin{split}
  \mathbf{V} &= (\mathbf{V_x}, \mathbf{V_y}) \\
  \mathbf{V\prime} &\subset \mathbf{V}\\
  \mathbf{V\prime} &=
  \begin{cases}
    \mathbf{V}, & \text{if } \|\mathbf{V}\| \geq \max( \|\mathbf{V}\|) \times 0.25 \\
