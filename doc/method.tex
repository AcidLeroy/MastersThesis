...
\begin{algorithmic}[1]
  ...
  \For{each \textit{video} in pre-segmented videos} \Comment{The following code occurs in the master node}
    \State $\texttt{UploadToS3(}\textit{video}\texttt{)}$
  \EndFor
  \State $\texttt{AddVideosToSqsQueue(}\textit{video\_list.txt}\texttt{)}$

  \Comment{The following code occurs in the compute nodes}
  ...
  \Comment{The remainder of the actions occur on the master node}
