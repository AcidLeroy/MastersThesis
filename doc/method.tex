\PARstart In this section we review the basic implementation details of our distributed
...
% and the psuedo-code for the entire system
...
%   \EndWhile
%
%   \State $\textit{output.txt} = \texttt{WriteCdfsToFile(} $
%   \Statex $\textit{PopAllCdfsFromQueue(}\textit{output\_queue\_name} \texttt{))}$
%
%   \State $\textit{dataframe} = \texttt{ReadCsv(} \textit{output.txt} \texttt{)}$
%
%   %\Statex  \Comment{Leave-one out algorithm for training and testing}
...
%     \State $\textit{svm\_model} = \texttt{svm(}\textit{train\_data, kernel=linear,}$
%     \Statex $\textit{cost = 0.001...10000} \textit{)}$
...
%       \State $continue$
%     \EndIf
%     \State $\textit{optical\_flow} = \texttt{CalculateOpticalFlow(} $
%     \Statex $\textit{current\_frame, previous\_frame} \texttt{)}$
...
%     \Statex $\textit{centroid\_x, centroid\_y, blob\_orientations, }$
%     \Statex $\textit{background\_mag, 25} \texttt{)}$
%   \EndFor
...
\subsubsection{\label{subsubsection:slave_node}Slave Node Configuration }
...
One of the hurdles in launching applications that run on a cluster is that
...
% \subsection{\label{subsection:auto_deployment}Automatic Deployment of Software and
% the Development Process}
% An aspect of this paper that separates us from many of the techniques proposed
% in the background subsection, is the way we have developed our system to be readily
...
% paper to get a head start on future developments of the proposed feature
% extraction algorithm. In this subsection we cover several topics that demonstrate
...
\subsubsection{\label{subsubsection:opencv_tapi}Vertical Scalability}
...
% \subsubsection{\label{subsubsection:cont_integration}Continuous Integration}
...
% interface.  A goal of this paper is to maximize reusability and maintainability. We
...
% immediately.
%
% For this paper, we chose to use an online tool called Travis-CI for our
...
% Figure \ref{fig:continuous_integration} illustrates the
...
% \subsubsection{\label{subsubsection:tdd}Test Driven Development}
...
% our method's subsection because repeatability of our experiments and future
...
% \end{itemize}
%
% For this paper we use TDD to develop our extract features program and to design
...
\Procedure{CalculateVectors}{$frame1$, $frame2$}
  \If {\texttt{$track\_points\_initialized$}}
...
% did using the Farneback method. Hence most of the results in this paper
% have been calculated with Farneback optical flow unless otherwise specified.

\subsection{\label{subsection:feature_extraction}Feature Extraction from Optical Flow}
...
space.
% Figure \ref{fig:life_of_video} illustrates how a video is first read in
% to extracting the features from each of the cropped videos produced.
